...
if __name__ == '__main__':
    docs = Docs(os.environ['DB_DBNAME'],
                os.environ['DB_SERVER'],
                os.environ['DB_USER'],
                os.environ['DB_PASS'],
                '{ODBC Driver 17 for SQL Server}',
                './doc.jsonl',
                './idx_fulltext_mapping.json',
                "idx_keyword_search")
    docs.gen_docs("SELECT [contentId],[attachments],[titel],[omschrijving],[tekst],[auteur],[jaarPublicatie],[pakketNamen],[themaNamen],[tagNamen],[documentNamen],[classificatieNaam],[typeNaam],[aangemaaktDoor],[aangemaaktOp],[duur],[beoordeling],[aantalBeoordelingen],[pageviews],[timestamp],[organisatieId] FROM [dbo].[vwElasticSearchIndex] WHERE [gepubliceerd] =1")
    docs.indexing_files()
